...
function shouldLink(name, repo) {
	...
	} else if (projects[name] && projects[name].exists) {
		verbose('>>> ' + name + ' exists locally');
		ok = true;
	}
	return ok;
}
...
function matchersFromFile(file, cb) {
	...
	childProcess.exec('git config --get remote.origin.url', { cwd: dir }, function (err, stdout, stderr) {
		if (err) {
...
function execCommand(project, cb) {
	...
	var proc = childProcess.spawn(argv[0], argv.slice(1), {
		cwd: project.path,
...
function gitStatus(project, output, cb) {
	...
	async.series([
		function (cb) {
			childProcess.exec('git rev-list --count @{u}..', { cwd: project.path }, function (err, stdout, stderr) {
				if (err) {
					failed = err;
				} else {
		...
		function (cb) {
			...
			var proc = childProcess.spawn('git', gitStatusArgs, {
				cwd: project.path,
				stdio: ['ignore', 'pipe', 'pipe']
			});
			proc.stdout.on('data', function (chunk) {
...
steps.deps = steps.common.concat([
	eachProject(function (project, cb) {
		...
		if (project.missing) {
			...
			var targetIsRegex = /[.+*()^$?]/.test(targetDependency);
			if (targetIsRegex) {
				if (argv[0] === '-u') {
					info("error: can't use regex dependency for update");
					process.exit(1);
				}
				var targetRe = new RegExp(targetDependency);
				_.each(project.dependencies, function (ver, dep) {
					if (targetRe.test(dep)) {
						output(dep + ': ' + ver);
					}
				});
			} else {
				...
				if (dependencyVersion) {
					if (argv[0] === '-u') {
						...
						writePkg.sync(path.join(project.path, 'package.json'), project.json);
					} else {
...
steps.intellij = steps.common.concat([
	function (cb) {
		function write(filename, content) {
			...
			if (noop) {
				childProcess.execSync('diff -N -u -w ' + filename + ' - || true',
		...
		_.each(projects, function (project) {
			...
			var libraryFile = path.join(project.path, '.idea/libraries/' + project.name.replace(/[ -]/g, '_') + '_node_modules.xml');
